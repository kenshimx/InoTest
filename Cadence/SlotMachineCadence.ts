...
function slotCadence(symbols: Array<SlotCoordinate>): SlotCadence {
  ...
  for (let i = 0; i < anticipatorConfig.columnSize; i++) {
    const symbolsInCurrentCol = symbols.filter(s => s.column == i).length;
    specialSymbolsFound += symbolsInCurrentCol;

    const isAnticipating = specialSymbolsFound >= anticipatorConfig.minToAnticipate && specialSymbolsFound < anticipatorConfig.maxToAnticipate;
    const currentInterval = isAnticipating ? anticipatorConfig.anticipateCadence : anticipatorConfig.defaultCadence;

    cadences.push(cadences[i] + currentInterval);
  }
